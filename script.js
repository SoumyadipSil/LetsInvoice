...
function calculateTotals() {
  ...
  const discountRaw = Math.max(0, Number(state.discount) || 0);
  const discount = Math.min(discountRaw, subtotal); // cap discount at subtotal
  const taxable = Math.max(0, subtotal - discount);
  const tax = taxable * (Number(state.taxRate) || 0) / 100;
...
function getInvoiceHTML() {
  ...
  // Logo helper — base64 src works in browser AND in html2canvas PDF export
  const logoHTML = state.business.logo
    ...
    : '';
  // Used by t2, t4 (dark header templates with chip boxes)
  const genericHeaderInfo = `
    <div class="chips">
      ...
      <div class="chip"><div class="cl">Date</div><div class="cv g">${escapeHTML(invoiceDate || '--')}</div></div>
      <div class="chip"><div class="cl">Due</div><div class="cv g">${escapeHTML(dueDate || '--')}</div></div>
      <div class="chip"><div class="cl">Status</div><div class="cv g">Unpaid</div></div>
    </div>`;
  ...
  const totalsBox = `
    <div class="tw-w"><div class="tw">
      ...
      <div class="tr-r"><span>Discount</span><span>${money(totals.discount)}</span></div>
      <div class="tr-r"><span>Tax (${Number(state.taxRate)||0}%)</span><span>${money(totals.tax)}</span></div>
  ...
  const footerBox = `
    <div class="ft">
      <div class="inv-foot">
        <div class="inv-foot-n"><strong>Notes</strong>${escapeHTML(state.notes || '')}</div>
        <div class="inv-foot-b">${escapeHTML(state.paymentDetails || '')}</div>
      </div>
    </div>`;

  // t5 uses .mfoot instead of .ft (sits inside flex column, margin-top:auto)
  const mfootBox = `
    <div class="mfoot">
      <div class="inv-foot">
        <div class="inv-foot-n"><strong>Notes</strong>${escapeHTML(state.notes || '')}</div>
  ...
  // t3 uses its own styled datebar instead of the chip-based genericHeaderInfo
  if (t === 't3') {
    const t3datebar = `<div class="datebar">
      ...
      <span class="due">Due: <strong>${escapeHTML(dueDate || '--')}</strong></span>
    </div>`;
    ...
    return `<div class="invoice-shell t3">
      <div class="h"><div class="biz-r">${t3avatar}<div><div class="biz-name">${escapeHTML(state.business.name || 'Your Business')}</div><div class="biz-sub">${escapeHTML(state.business.email || '')}</div></div></div><div><div class="inv-word">Invoice</div><div class="inv-num">${escapeHTML(state.invoice.number || 'INV-001')}</div></div></div>
      ${t3datebar}
      ${commonParties}
      <div class="iw"><table class="inv-t"><thead><tr><th>Description</th><th>Qty</th><th>Rate</th><th>Amount</th></tr></thead><tbody>${lines}</tbody></table></div>
      ${totalsBox}
  ...
  if (t === 't4') {
    return `<div class="invoice-shell t4">
      <div class="h"><div class="h-top"><div>${logoHTML}<div class="biz-name">${escapeHTML(state.business.name || 'Your Business')}</div><div class="biz-sub">${escapeHTML(state.business.email || '')}${state.business.gst ? ` · GSTIN ${escapeHTML(state.business.gst)}` : ''}</div></div><div><div class="inv-lbl">Tax Invoice</div><div class="inv-num">${escapeHTML(state.invoice.number || 'INV-001')}</div></div></div>${genericHeaderInfo}</div>
      ${commonParties}
      <div class="iw"><table class="inv-t"><thead><tr><th>Description</th><th>Qty</th><th>Rate</th><th>Amount</th></tr></thead><tbody>${lines}</tbody></table></div>
      ${totalsBox}
      ${footerBox}
  ...
  if (t === 't5') {
    ...
    return `<div class="invoice-shell t5">
      <div class="side">
        ...
        <div><div class="slbl">Bill to</div><div class="sname">${escapeHTML(state.client.name || 'Client')}</div><div class="ssdet">${escapeHTML(state.client.address || '')}</div></div>
        <div><div class="slbl">Invoice</div><div class="ppill"><div class="pline">${escapeHTML(state.invoice.number || 'INV-001')}</div><div class="pline">${escapeHTML(invoiceDate || '--')}</div><div class="pline">Due ${escapeHTML(dueDate || '--')}</div></div></div>
      </div>
      <div class="main"><div class="inv-word">Invoice</div><div class="inv-num">${escapeHTML(state.invoice.number || 'INV-001')}</div><div style="margin-top:16px"><table class="inv-t"><thead><tr><th>Description</th><th>Qty</th><th>Rate</th><th>Amount</th></tr></thead><tbody>${lines}</tbody></table></div>${totalsBox}${mfootBox}</div>
    </div>`;
  }
  ...
  if (t === 't6') {
    return `<div class="invoice-shell t6">
      ...
      <div class="datebar"><span>Issued:</span><span>${escapeHTML(invoiceDate || '--')}</span><span>Due:</span><span>${escapeHTML(dueDate || '--')}</span><span>Currency:</span><span>${escapeHTML(state.currency)}</span></div>
      ${commonParties}
      <div class="iw"><table class="inv-t"><thead><tr><th>Description</th><th>Qty</th><th>Rate</th><th>Amount</th></tr></thead><tbody>${lines}</tbody></table></div>
      ${totalsBox}
      ${footerBox}
  ...
  if (t === 't7') {
    return `<div class="invoice-shell t7">
      <div class="h"><div>${logoHTML}<div class="biz-name">${escapeHTML(state.business.name || 'Your Business')}</div><div class="biz-sub">${escapeHTML(state.business.email || '')}</div></div><div><div class="inv-lbl">Invoice</div><div class="inv-num">${escapeHTML(state.invoice.number || 'INV-001')}</div></div></div>
      ${commonParties}
      <div class="iw"><table class="inv-t"><thead><tr><th>Description</th><th>Qty</th><th>Rate</th><th>Amount</th></tr></thead><tbody>${lines}</tbody></table></div>
      ${totalsBox}
      ${footerBox}
  ...
  return `<div class="invoice-shell t1">
    <div class="h"><div>${logoHTML}<div class="biz-name">${escapeHTML(state.business.name || 'Your Business')}</div><div class="biz-sub">${escapeHTML(state.business.email || '')}${state.business.gst ? ` · GSTIN ${escapeHTML(state.business.gst)}` : ''}</div></div><div><div class="inv-word">INVOICE</div><div class="inv-num">${escapeHTML(state.invoice.number || 'INV-001')}</div></div></div>
    ${commonParties}
    <div class="iw"><table class="inv-t"><thead><tr><th>Description</th><th>Qty</th><th>Rate</th><th>Amount</th></tr></thead><tbody>${lines}</tbody></table></div>
    ${totalsBox}
    ${footerBox}
...
function syncInputsToState() {
  ...
  state.taxRate = Number(val('taxRate') || 0);
  state.discount = Number(val('discount') || 0);
...
async function downloadPDF() {
  ...
  pdf.save((state.invoice.number || 'invoice').replace(/[^a-z0-9-_]+/gi, '_') + '.pdf');
}
